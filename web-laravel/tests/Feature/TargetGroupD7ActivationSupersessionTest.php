...
final class TargetGroupD7ActivationSupersessionTest extends TestCase
{
    ...
    {
        $f = $this->fixture();
        DB::table('target_group_rows')->insert(['target_group_job_id' => $f['job'], 'target_group_file_id' => $f['file'], 'sheet_name' => 'Sheet1', 'row_number' => 1, 'raw_payload' => json_encode(['synthetic' => true]), 'cid_status' => 'NOT_EVALUATED', 'validation_status' => 'PENDING']);
        $row = (int) DB::table('target_group_rows')->where('target_group_file_id', $f['file'])->value('id');
        DB::table('target_group_row_reviews')->insert(['target_group_job_id' => $f['job'], 'target_group_file_id' => $f['file'], 'target_group_row_id' => $row, 'correlation_id' => (string) Str::uuid(), 'from_status' => 'PENDING_VALIDATION', 'to_status' => 'NEEDS_REVIEW', 'review_reason_code' => 'SYN_REASON', 'created_at' => now()]);
    ...
    private function fixture(): array
    {
        ...
        $job = $this->createJob('SYN_CANDIDATE'); $file = $this->createFile($job, 'candidate.csv', 'SYN_CANDIDATE'); $candidate = $this->createVersion($lineage, $job, $file, 'CANDIDATE', 2, $predecessor);
